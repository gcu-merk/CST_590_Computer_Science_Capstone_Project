...
get_detection_data() {
    if [ -n "$SPECIFIC_CORRELATION_ID" ]; then
        ...
        recent_detection=$(docker exec database-persistence sqlite3 /app/data/traffic_data.db \
            "SELECT timestamp, vehicle_type, correlation_id FROM traffic_detections WHERE correlation_id='$SPECIFIC_CORRELATION_ID' ORDER BY timestamp DESC LIMIT 1;" 2>/dev/null)
    else
...
main() {
    ...
    if [ -n "$recent_detection" ]; then
        ...
        echo -e "  🚗 Vehicle: ${GREEN}$vehicle_type${NC}"
        ...
        echo -e "  🎯 Result: Vehicle classified as ${GREEN}$vehicle_type${NC}"
